...
def render_full_interpretation(label, param_type, detail_df, shape, reference_period=None,
    ...
    if asymmetric:
        ...
        if gwp_rise > gwp_drop:
            variants = [
                f"<b>{display_label}</b> is a one-way risk. Pushing it above baseline costs more GWP "
                f"({gwp_rise:.2f} kg CO₂-eq/m²) than an equivalent reduction below baseline saves "
                f"({gwp_drop:.2f} kg CO₂-eq/m²). If your estimate of {asym_kind} is uncertain or "
                ...
                f"The risk on <b>{display_label}</b> isn't symmetric. Going over baseline adds "
                f"{gwp_rise:.2f} kg CO₂-eq/m², while the same-sized move under baseline only "
                f"saves {gwp_drop:.2f}. So if there's any real doubt about {asym_kind}, a "
                f"cautious (lower) number is the safer guess to work with, since overshooting costs "
                f"more than undershooting saves.",

                f"Worth flagging for <b>{display_label}</b>: overshooting baseline is the expensive "
                f"direction here ({gwp_rise:.2f} kg CO₂-eq/m² added), while undershooting by the "
                f"same amount only gives back {gwp_drop:.2f}. If {asym_kind} tends to run higher "
            ...
            variants = [
                f"<b>{display_label}</b> is a one-way opportunity: reducing it below baseline saves more "
                f"GWP ({gwp_drop:.2f} kg CO₂-eq/m²) than an equivalent increase above baseline "
                f"costs ({gwp_rise:.2f} kg CO₂-eq/m²). If you're looking for where to focus "
                f"reduction efforts, this asymmetry means the payoff here is bigger than the "
                f"symmetric numbers alone would suggest.",

                f"<b>{display_label}</b> rewards going low: trimming {asym_kind} below baseline saves "
                f"{gwp_drop:.2f} kg CO₂-eq/m², more than the {gwp_rise:.2f} kg CO₂-eq/m² an "
                f"equal-sized increase would cost. If this element is a candidate for reduction "
                f"efforts, this is one of the parameters where pushing the value down pays off "
                f"disproportionately.",

                f"There's an asymmetric upside on <b>{display_label}</b>: cutting it below baseline "
                f"saves {gwp_drop:.2f} kg CO₂-eq/m², while raising it by the same margin only "
                f"costs {gwp_rise:.2f}. Worth keeping in mind if you're hunting for reduction "
