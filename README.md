...
- **Python 3.11 or newer** ([download it here](https://www.python.org/downloads/) if you don't have it)
...
**To stop the tool** when no analysis is running, press `Ctrl+C` in the terminal, or
just close the terminal window. If an analysis is currently running, use the app's own
**⏹ Stop** button first (see [step 7](#stop-button) below), then close the terminal. If the app isn't
responding at all, see "Chrome window seems frozen" under [7. If something goes
wrong](#7-if-something-goes-wrong) instead.
...
7. <a id="stop-button"></a>Pick a parameter or two and start the Full Analysis. The tool goes through them
...
10. Want to try a value without committing to it? Open **Parameter Explorer**, change
    ...
    in eLCA automatically afterward, including if an analysis step fails in a way the
    tool can catch; use **↺ Reset to baseline values** to put every field back to its
    real starting value.
...
good news / reassuring, **purple** = neutral, worth knowing, **amber** =
...
Every value the tool changes gets reset back to its original state automatically,
including when an analysis step fails in a way the tool can catch, everywhere in the
tool, not just Parameter Explorer. The one gap is the process itself getting killed or
crashing outright before it can clean up; see [7. If something goes
wrong](#7-if-something-goes-wrong) for what that looks like and how to fix it.
...
- Element names sometimes show up in German instead of English: this tool only
  ...
  eLCA wrote it.
- The tool currently analyzes GWP only. eLCA computes several other environmental
  indicators that aren't covered yet.
...
Every GWP value the analysis works from is read directly from eLCA through this chain.
From those raw measurements, the tool derives everything else itself: relative ranges,
sensitivity rankings, response shapes, interaction percentages, and the plain-language
interpretations shown alongside them.
